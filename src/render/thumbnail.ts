...
export function blockThumbnail(editor: Editor, blockId: Id, size = 96, dark = false): string | null {
  const block = editor.doc.data.blocks.get(blockId);
  if (!block || typeof document === 'undefined') return null;
  const key = `${blockId}|${block.revision}|${editor.ctx.blocksVersion}|${size}|${dark}`;
  const hit = cache.get(key);
  if (hit) return hit;
  const ev = editor.ctx.evaluateBlock(blockId);
  const box: BBox = emptyBox();
  for (const e of ev.entities) {
    try {
      const b = kindOf(e).bbox(e, editor.ctx);
  ...
  if (!isEmptyBox(box)) {
    ...
    const env: TraverseEnv = { doc: editor.doc, ctx: editor.ctx, dark, background: 'transparent', plotting: false, plotStyle: 'color', viewport: null, dashScale: 1, forceColor: dark ? '#f2f4f3' : '#14171a' };
    for (const e of ev.entities) {
